...
var ans = document.getElementsByClassName("card-body");
var r = document.querySelector(':root');

function swtichTheme() {

  setCookie("Theme", swi.value, 100);
  if (swi.value === "Dark"){
    r.style.setProperty('--CardBodyBk','#bcc1c4');
    r.style.setProperty('--CardBodyColor','#000000');
    r.style.setProperty('--NamesColor','#F7F7F7');
    r.style.setProperty('--CardColorBK','#1b524c');
    r.style.setProperty('--CardLink','rgb(209, 209, 209)');
    r.style.setProperty('--DocumentBKcolor','#121212');
    r.style.setProperty('--TopLabelColor','#FFFFFF');
    r.style.setProperty('--SwitchedOff','rgb(30, 30, 30)');
    r.style.setProperty('--SwitchedOn','rgb(9, 61, 39)');
    r.style.setProperty('--FlexingTitleBk','rgb(13, 61, 61)');
  }else{

    r.style.setProperty('--CardBodyBk','#FFFFFF');
    r.style.setProperty('--CardBodyColor','#000000');
    r.style.setProperty('--NamesColor','#000000');
    r.style.setProperty('--CardColorBK','#A9A9A9');
    r.style.setProperty('--CardLink','#000000');
    r.style.setProperty('--DocumentBKcolor','#FFFFFF');
    r.style.setProperty('--TopLabelColor','#000000');
    r.style.setProperty('--SwitchedOff','rgb(247, 247, 247)');
    r.style.setProperty('--SwitchedOn','rgb(157, 255, 195)');
    r.style.setProperty('--FlexingTitleBk','rgb(25 63 33)');
  }
}

  for (var i = 1; i <= ans.length; i += 1) {
    var ckboxVal = document.getElementById("ck" + i);
    ckboxVal.checked = getCookie("ck" + i) == 1 ? true : false;
    ckBox(i,ckboxVal);
  }
  
function ckBox(id, checkbox) {
  setCookie("ck" + id, checkbox.checked ? 1 : 0, 100);
  const header = document.getElementsByClassName("card-header head" + id);
  if (checkbox.checked) {
    console.log(swi.value);
    header[0].classList.remove("SwitchedOff");
    header[0].classList.add("SwitchedOn");
  }
  else{
    header[0].classList.remove("SwitchedOn");
    header[0].classList.add("SwitchedOff");
  }
}
...
function onload() {
  ...
  if (Themecookie == null) swi.value = "Dark";
  else swi.value = Themecookie;
  swtichTheme();
}
onload();